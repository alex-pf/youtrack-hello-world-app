 ...
 * @param showProjectedLT - Whether to show the projected lead time marker; also
 *   gates parsing of estimate date change history, which
 *   calculateProjectedLeadTime uses as a fallback when the current Estimated
 *   Date field value isn't directly readable from issue.fields.
 * @param groupByField - Technical/localized name of the enum field to group by
 *   (widget config's groupByField); resolved via extractGroupFieldValue into
 *   the returned groupValue.
...
export function buildIssueChartData(
  ...
  // Parse estimate date changes when needed — only showProjectedLT gates this
  // now; the new widget has no separate showEstimateDate UI setting (history
  // display was dropped, see docs/PROGRESS_TRACKING_SPEC.md section 7), but
  // the array is still needed internally as calculateProjectedLeadTime's
  // fallback source.
  const estimateDateChanges = showProjectedLT
  ...
  const groupValue = extractGroupFieldValue(issue, groupByField);
...
/**
 * Sorts chartData in place per the given SortBy. Exported (unlike the donor
 * widget's module-private sortChartData) so it can be reused both here and
 * per-group inside groupChartData below.
 */
export function sortChartDataInPlace(chartData: IssueChartData[], sortBy: SortBy): void {
  switch (sortBy) {
    case 'issueNumber':
 ...
 * Includes issues even with no segments (they'll show as empty rows) — the
 * primary/additional search filtering has already happened at the REST-query
 * level (resources.ts), so nothing is filtered out here.
 *
 * Unlike the donor widget, this does NOT sort the returned list itself —
 * final ordering is per-group, applied by groupChartData below. Callers that
 * need a flat sorted list (no grouping) should call sortChartDataInPlace
 * directly.
 *
 ...
 * @param groupByField - Technical/localized name of the enum field to group by
 * @returns Array of IssueChartData, unsorted (processing order)
 */
export function buildChartData(
  ...
  for (const issue of issues) {
    ...
    const data = buildIssueChartData(issue, activities, statusOrder, showProjectedLT, groupByField);
...
/**
 * Groups chart data by groupValue and sorts each group's items independently
 * (per sortBy), rather than sorting the flat list globally.
 *
 * Implementation default: issues with no groupValue (field absent from the
 * project, or present but unset on that issue) are NOT dropped — they're
 * collected into a group keyed by the empty string ''. This is a deliberate
 * implementation-level choice (not mandated by the spec's "exclude only
 * explicitly-flagged empty optional items" language) — an issue's group
 * membership is a fact about the issue,  not an optional per-chart setting,
 * so we surface it as an explicit "(no value)" group rather than silently
 * losing rows. The render layer (increment 6/7) decides how to label/order
 * this group among the others.
 *
 * Map key order: keys are inserted in ascending localeCompare order (over
 * the set of distinct groupValues, '' included) so that iterating the
 * returned Map yields a predictable, stable chart order without requiring
 * every caller to re-sort the keys themselves. Final visual ordering of
 * per-group charts (if different from alphabetical) is a render-layer
 * concern for a later increment.
 *
 * @param chartData - Flat array from buildChartData
 * @param sortBy - Row order applied WITHIN each group; defaults to 'startDate'
 * @returns Map of groupValue (or '' for no value) → sorted IssueChartData[]
 */
export function groupChartData(
  chartData: IssueChartData[],
  sortBy: SortBy = 'startDate'
): Map<string, IssueChartData[]> {
  const byGroup = new Map<string, IssueChartData[]>();
  ...
  const sortedKeys = Array.from(byGroup.keys()).sort((a, b) => a.localeCompare(b));

  const result = new Map<string, IssueChartData[]>();
  for (const key of sortedKeys) {
    const items = byGroup.get(key)!;
    sortChartDataInPlace(items, sortBy);
